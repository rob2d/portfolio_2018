...
import getFontHeight from './getFontHeight';
...
export default class CanvasText {
    ...
    drawText(text, ctxOptions) {
        ...
        this.textWidth = Math.max.apply(null,
        ...
        this.canvas.width = Math.max(2,
            MathUtils.ceilPowerOfTwo(this.textWidth + (2 * ctxOptions.horizontalPadding))
        );
        this.canvas.height = Math.max(2,
            MathUtils.ceilPowerOfTwo(this.textHeight + (2 * ctxOptions.verticalPadding))
        );
        ...
        if(ctxOptions.backgroundColor) {
            ...
            this.ctx.fillRect(
                0,
                0,
                this.textWidth + (2 * ctxOptions.horizontalPadding),
                this.textHeight + (2 * ctxOptions.verticalPadding)
            );
        }
        ...
        for(let i = 0; i < lines.length; i++) {
            this.ctx.fillText(
                lines[i],
                x + ctxOptions.horizontalPadding,
                (lineHeight * ctxOptions.lineHeight * i) + ctxOptions.verticalPadding + y
            );
        }
        return this.canvas;
    }
